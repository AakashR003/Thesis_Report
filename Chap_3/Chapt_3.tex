...
less severe than that of spectral leakage, provided the signal frequency falls on a frequency
bin.
...
the total record length is extended to 20~s, the bin spacing is reduced, placing the
...
to mitigate error in the limiting case, where the frequency falls exactly at the midpoint
between two bins, is to sum the amplitudes of the two neighbouring bins. However, this method
...
through a longer data record reduces spectral leakage error by moving the signal frequency
closer to a bin in relative terms. However, as the resolution increases, the sensitivity of
the error also increases. Consequently, while a finer frequency
resolution is generally beneficial for accuracy, its associated increase in error sensitivity
must be carefully considered when increasingthe length.
