...
This chapter presents a series multi-frequency forced-motion CFD simulations performed to answer the questions 
...
earlier. These parameters are then applied in two distinct multi-frequency campaigns in which the frequency superpositions 
kept the same but the prescribed motion amplitudes are varried. Finally, based on the results and inferences obtained from both campaigns are 
, practical framework for the selection of frequencies and amplitudes in multi-frequency simulations is proposed. 

The chapter proceeds as follows. Section 1 defines the simulation parameters, including the selection of frequencies and 
the superposition schemes. Section 2 presents the first multi-frequency campaign together with the detailed procedure of 
the multi-frequency method using the fast Fourier transform. Section 3 describes the second multi-frequency campaign. 
...
organized into four superposition schemes. Superposition scheme~1 consists of single-frequency 
...
must be a definite number, so that the dependent frequencies fall exactly on the frequency bins. To illustrate, 
if the lowest frequency is set to 0.133.. Hz, the resulting frequency bins would be 0.133.. Hz, 0.266.. Hz, 0.40 Hz, 
and so on (determined by multiplying by integer values). When the integer is a multiple of 3, the bins yield exact values; 
otherwise, they produce inexact numbers. For instance, if a dependent frequency within the set is set to 0.266 Hz, 
the mismatch between the intended value (0.266) and the bin value (0.26666...) will prevent proper alignment with the bin, 
resulting in spectral leakage that causes error in the results. To avoid this issue, it is essential to select the lowest 
...
For both campaigns in this investigation, the lowest frequency is set to $0.125$~Hz ( 
...
Note that the frequency spacing and frequency bins depend on the number of time steps. Therefore, if the simulation 
...
A 34 CFD simulations is conducted and the results discussed in the current and following sections. 
...
this behavior is investigated further in Section~\ref{sec:MF1_FreqDomain}. For pitch motion, 
...
Consequently, the multi-frequency method for H2 and A2 derivatives can never yield results fully equivalent to single-frequency 
simulation results when the amplitude choosen for pitch is in the range of phase variations.\\
...
It shall be observed nonlinearity prevails in the lower reduced velocities at 4th superposition scheme but no consistent relationship 
or reasoning shall be made only from error analysis and linearity assesment. The reasoning for this behaviour is explained
with results of spectral analysis in the next subsection. 
...
the nonlinearity due to phase of pitch motion of the cross-section.\\
...
terms induced by large heave and pitch amplitudes, or from the nonlinearity due to phase of pitch motion of the 
...
the spectral analysis. The nonlinearity due to the phase of pitch motion of the cross-section therefore cannot 
...
amplitudes below the linearity limit obtained from the nonlinearity study. Hence the summation of amplitude within a superposition 
scheme will exceed the linearity limit. Similar to the post-processing of previous section on Multi-Frrequency simulation 1 here also 
carried out and the results that only differ from it are presented. 
...
comparatively little variation. The key observation here is that the keeping only individual amplitude below the nonlinearity threshold 
...
as shown in the multi-frequency simulation - 1.\\
...
because of nonlinearity due to the phase of pitch motion of the cross-section.\\
...
An important conclusion from this section is that the spectral analysis is a
...
frequencies up to a superposition of three frequencies. The derivatives $H_4$ and $A_4$ exhibit slight errors, while $H_2$ 
and $A_2$ show the most significant deviations. Despite maintaining the amplitude summation below the linearity threshold, 
considerable deviations appear in the flutter derivatives A2 and H2 across superposition schemes. These deviations are due to phase 
changes identified in the amplitude study of Chapter 4. A important consideration is determining which result is representative. 
Becase, in real-world scenarios, externally applied motion is typically not purely sinusoidal; therefore, the 
multi-frequency approach provides a comparably better representation of flutter derivatives $H_2$ and $A_2$ that captuirng variations due to phase. Also, 
these derivatives have minimal influence on the critical wind speed. However, as this conclusion is based on only four bridge 
deck configurations, it should not be generalized. For other bridge deck properties, these derivatives may be critical. 
...
From multi-frequency simulation Campaign 2, if the sum of amplitude exceeds the linearity threshold, the flutter derivatives 
become highly inaccurate. Importantly, this nonlinearity is detectable in spectral analysis. Therefore, people using 
...
Accurate flutter derivative results are limited to superpositions of up to three prescribed motions. At the fourth prescribed 
motion, considerable inaccuracy appears across all derivatives and significantly affects the identified critical velocity. 
This is due to high ratio of aerodynamic forces between frequency components within a superposition. These 
force ratios become high at lower reduced velocities. Notably, this limitation is not visible in spectral 
analysis and is localized to specific reduced velocities where force amplitudes are small. It is therefore recommended to 
plot aerodynamic forces versus reduced velocity after multi-frequency simulations to verify that force ratios remain acceptably small. 
...
forces exhibit minimal variation. This is advantageous for the multi-frequency method 
because the primary objective of multi-frequency method is reducing simulation time. Higher reduced velocities, corresponds to lower 
frequencies, requires more time steps and simulation times. For example, superposing the 
two highest reduced velocities can reduce twice the simulation times compared to combining four lower reduced velocities. 
...
range, while single-frequency simulations are advisable for the lower reduced velocity range. For heave versus pitch motion 
comparisons, the choice between single and multi-frequency methods is subjective.
...
velocities yields the highest confidence in derivative accuracy. For pitch motion with the four highest frequencies, results 
...
\begin{enumerate}
    \item \textbf{Determine the number of superpositions.} The recommended number is 3 or 4. Based on the 
          ...
          accurate flutter derivatives across all frequencies. At the fourth prescribed motion, considerable 
          inaccuracy begins to appear in the lower reduced velocity range, 
          which can significantly affect the identified critical velocity. The limitation governing the maximum 
          number of superpositions is governed by the relative aerodynamic forces between frequency components 
          within a superposition. Also superposing more than four frequencies does not account for high reduction in 
          compuatational cost. This is because, when we superpose higher frequencies (lower reduced velocities), only 
          lesser time steps are saved (\ref{fig:Overall Combinations - Total Time Saved vs No. of Superpositions})

    \item \textbf{Target the lower frequency range.} Superpositions should preferably be used at the lowest 
          frequencies (i.e.\ those at the highest reduced velocities). This approach provides two 
          advantages: first, it reduces error contributions that are prevalent at lower reduced velocities where 
          force amplitude ratios become large; second, it reduces overall simulation time. The 
          primary reason for high computational time are the lower frequencies (higher reduced velocities), 
          which require more time steps. So by superposing the lower reduced velocities, simulation time can be 
          reduced twice as compared to superposing four high frequencies (lower reduced velocities) 
    ...
    \item \textbf{Select one independent frequency per superposition.} The independent frequency (lowest frequency) 
          ...
          bins and stable time period. This frequency should be chosen such that its frequency bin spacing 
          produces a definite number, ensuring that all dependent frequencies fall exactly on the frequency bins. 


    \item \textbf{Dependent frequencies} Derive the dependent frequencies within each superposition such that they fall exactly within 
          the frequency bin of the independent frequency. A detailed procedure for this calculation is shown at 
          the start of this chapter. When selecting frequencies for superposition schemes, it is essential 
          to avoid choosing two frequencies that are consecutive in the frequency bin list. If one frequency does 
          not align with a bin due to incorrect selection, the result at the neighbouring frequency will get affected spectral leakage. 
    ...
    \item \textbf{Selection of  amplitudes} Amplitudes must be selected such that the total amplitude (sum across all frequencies in a superposition) 
    ...
    \item \textbf{Post-processing and verification:} Apply the Discrete Fourier Transform (DFT) to the 
          aerodynamic forces and moments for extraction individual frequency components. The frequency spectrum must 
          be analysed for signs of nonlinearity, which due to higher-order harmonic terms or as spectral leakage. 
