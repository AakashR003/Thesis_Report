...
This chapter presents a series multi-frequency forced-motion CFD simulations performed to resolve the questions 
...
\begin{flushleft}
\hspace{2cm}1. Superposition Schemes \\
\hspace{2cm}2. Comparison of Flutter Derivatives \\
\hspace{2cm}3. Spectral Analysis \\
\hspace{2cm}4. Error Assessment Against Single Frequency Simulations \\
\hspace{2cm}5. Superposition Impact on Critical Wind Speed \\
\end{flushleft}
...
considerable deviations appear in the flutter derivatives across superposition schemes. These deviations are due to phase 
changes identified in the amplitude study of Chapter 4. A critical consideration is determining which result is correct. 
In real-world scenarios, externally applied motion is typically not purely sinusoidal; therefore, the 
...
\begin{enumerate}
    ...
    \item \textbf{Target the lower frequency range.} Superpositions should preferably be formed from the lowest 
          frequencies (i.e.\ those at the highest reduced velocities). This approach provides two significant 
          advantages: first, it reduces error contributions that are prevalent at lower reduced velocities where 
          force amplitude ratios become large; second, it substantially reduces overall simulation time. The 
          primary drivers of extended computational time are the lower frequencies (higher reduced velocities), 
          which require more time steps. By combining the highest reduced velocities, simulation time can be 
          reduced substantially compared to combining four lower reduced velocities. 
