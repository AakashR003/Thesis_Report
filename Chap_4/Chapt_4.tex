...
A comparable phase deviation phenomenon has been reported in previous studies (--- and ---). 
However, in those works the noticeable change in phase typically appeared at significantly larger pitch 
angles than observed here. This difference prompts the question of whether the underlying physical mechanism 
is the same or whether distinct flow phenomena are responsible.

In the referenced studies, the authors attributed the phase shift primarily to changes in the flow reattachment 
location and the resulting variation in pressure distribution along the section. At small pitch amplitudes, 
the pressure field remains relatively consistent throughout the oscillation cycle — the flow does not undergo 
substantial changes between the positive and negative peaks. At larger pitch amplitudes, however, the pressure 
distribution varies markedly over the cycle because the flow field experiences much stronger alterations during 
the motion. This increased unsteadiness leads to the observed phase deviation.
...
to cover one full oscillation cycle, starting from the negative peak displacement at approximately 30 seconds 
(see the prescribed motion in Figure~\ref{fig:Prescribed Displacement}).

Figure~\ref{fig: Amplitude Study - 0 - Pitch Motion - Pressure Distribution} presents the pressure distribution 
around the cross-section at several time steps for the smallest amplitude case (Amplitude 0). The pressure field 
remains quite consistent across the different phases of the motion. By contrast, Figure~\ref{fig: Amplitude Study - 6 - Pitch Motion - Pressure Distribution} 
shows the pressure distribution for the largest amplitude case (Amplitude 6). Here, the pressure field exhibits clear and substantial 
variation from one time step to another, indicating significant changes in the flow topology during the oscillation cycle.
This behaviour predominantly affects the pitching moment (and therefore derivatives $A_2$ and $H_2$) 
rather than the lift force (related to $A_3$ and $H_3$). Although the integrated lift force remains approximately linear 
even at higher pitch angles, the spatial distribution of pressure changes considerably. As a result, the effective centre 
of pressure (the moment arm weighted by the pressure distribution) shifts noticeably during the cycle, which is directly 
reflected in the pitching moment. The lift force, being the net vertical integration, is much less sensitive to these 
internal redistributions.
...
at which phase nonlinearity becomes significant is expected to vary across different cross-sectional shapes.
